...
class Calendar extends Component{
  ...
  //  LAST_DAY = 31
   PI = Math.PI
   currentLunarDate = this.getLunarDate(today.getDate(), today.getMonth()+1, today.getFullYear())
...
//    today = new Date()
   ...
   jdn2date(jd) {
    ...
    if (leapMonth == 0) {
      for(mm = 1; mm <= 12; mm++) {
        ly.push(this.LunarDate(1, mm, yy, 0, currentJD));
      ...
      ly.push(this.LunarDate(1, leapMonth, yy, 1, currentJD));
    ...
    var yearCode;
    if (yyyy < 1300) {
      yearCode = data.TK13[yyyy - 1200];
    } else if (yyyy < 1400) {
      yearCode = data.TK14[yyyy - 1300];
    } else if (yyyy < 1500) {
      yearCode = data.TK15[yyyy - 1400];
    } else if (yyyy < 1600) {
      yearCode = data.TK16[yyyy - 1500];
    } else if (yyyy < 1700) {
      yearCode = data.TK17[yyyy - 1600];
    } else if (yyyy < 1800) {
      yearCode = data.TK18[yyyy - 1700];
    } else if (yyyy < 1900) {
      yearCode = data.TK19[yyyy - 1800];
    } else if (yyyy < 2000) {
      yearCode = data.TK20[yyyy - 1900];
    } else if (yyyy < 2100) {
      yearCode = data.TK21[yyyy - 2000];
    } else {
      yearCode = data.TK22[yyyy - 2100];
    }
    ...
    var off = jd - ly[i].jd;
    let ret = this.LunarDate(ly[i].day+off, ly[i].month, ly[i].year, ly[i].leap, jd);
    return ret;
  }
   ...
   getYearCanChi(year) {
    return data.CAN[(year+6) % 10] + " " + data.CHI[(year+8) % 12];
  }

   getCanChi(lunar) {
    var dayName, monthName, yearName;
    dayName = data.CAN[(lunar.jd + 9) % 10] + " " + data.CHI[(lunar.jd+1)%12];
    monthName = data.CAN[(lunar.year*12+lunar.month+3) % 10] + " " + data.CHI[(lunar.month+1)%12];
    if (lunar.leap == 1) {
      monthName += " (N)";
    }
   ...
   getDayString(lunar, solarDay, solarMonth, solarYear) {
    var s;
    var dayOfWeek = data.TUAN[(lunar.jd + 1) % 7];
   ...
   getTodayString() {
    var s = this.getDayString(this.currentLunarDate, today.getDate(), today.getMonth()+1, today.getFullYear());
   ...
   getDateString(dd, MM, yyyy) {
    ...
    var s;
    var dayOfWeek = data.TUAN[(InputLunarDate.jd + 1) % 7];
   ...
   getGioHoangDao(jd) {
    ...
    var gioHD = data.GIO_HD[chiOfDay % 6]; // same values for Ty' (1) and Ngo. (6), for Suu and Mui etc.
    var ret = "";
    var count = 0;
    for (var i = 0; i < 12; i++) {
      if (gioHD.charAt(i) == '1') {
   ...
   printHead(mm, yy) {
    ...
    switch ( settings.type ) {
      ...
      case 'calendar':
        var cc = this.getCanChi(this.currentLunarDate),
            holiday = this.getHolodayString( today.getDate(), (today.getMonth()+1), this.currentLunarDate.day, this.currentLunarDate.month );
        res += '<tr>\n';
        res += '  <td colspan="7">\n';
        ...
        res += '            <span class="day-tuan">'+data.TUAN[(this.currentLunarDate.jd + 1) % 7]+'</span>\n';
        res += '          </td>\n';
        res += '        </tr>\n';
        res += '        <tr>\n';
        res += '          <td width="50%" class="calendar-b-left" valign="top">\n';
        res += '            <span class="lunar-month-name">Tháng '+data.THANG[this.currentLunarDate.month-1]+'</span><br>\n';
        ...
        res += '            <span>Giờ đầu <strong>'+(this.getCanHour0(this.currentLunarDate.jd)+' '+data.CHI[0])+'</strong></span><br>\n';
        res += '            <span>Tiết <strong>'+data.TIETKHI[this.getSunLongitude(this.currentLunarDate.jd + 1, 7.0)]+'</strong></span><br>\n';
    ...
    res += '<tr>\n';
    for(var i=0;i<=6;i++) {
      res += '  <td class="ngaytuan">'+data.DAYNAMES[i]+'</td>\n';
    }
    res += '</tr>\n';
    return res;
  }
   ...
   checkHolidaySolar( dd, mm ) {
    var res = '';
    data.LE.solar.forEach(  function( k, item ) {
   ...
   checkHolidayLunar( dd, mm ) {
    var res = '';
    data.LE.lunar.forEach(  function( k, item ) {
      if ( item.d == dd && item.m == mm ) {
